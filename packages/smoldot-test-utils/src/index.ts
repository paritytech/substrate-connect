...
const NO_MORE_RESPONSES =
  "json_rpc_callback was called but there are no mock responses left to respond with"
 ...
 * "system_health" RPC requests
 */
export const smoldotSpy = (
  responder: RpcResponder,
  rpcSpy: jest.MockedFunction<(rpc: string) => void>,
  healthResponder = healthyResponder,
): Smoldot => {
  return {
    start: (): Client => {
