...
describe("Initialize Detector without extension", () => {
  ...
  test(
    'Should connect with known chain "westend".',
    async () => {
  ...
  test(
    'Should connect with known chain "polkadot".',
    async () => {
      const detect = new Detector("test-uapp")
  ...
  test(
    "Should connect with known chain westend, no chainSpecs and options",
    async () => {
  ...
  test(
    'Should connect with known chain "kusama".',
    async () => {
      const detect = new Detector("test-uapp")
  ...
  test(
    "Should NOT connect with unknown chain westend2 and without chainSpecs.",
    () => {
      const chainName = "westend"
      const detect = new Detector("test-uapp")
      void expect(detect.connect(chainName)).rejects.toThrow(
        `No known Chain was detected and no chainSpec was provided. Either give a known chain name ('polkadot', 'kusama', 'westend') or provide valid chainSpecs.`,
      )
    },
    timeout,
  )
  ...
  test(
    'Should connect with known chain "westend" and parachain "westmint".',
    async () => {
      const detect = new Detector("test-uapp")
